...
MODEL_PATH = BASE_DIR / "models" / "gigafactory_gat_model.pth"
DB_PATH = BASE_DIR / "database" / "gigafactory_live.db"
USERS_PATH = BASE_DIR / "config" / "users.json"

LOG_DIR = BASE_DIR / "logs"
LOG_DIR.mkdir(exist_ok=True)
ACCESS_LOG_PATH = LOG_DIR / "access_requests.log"
...
def import_users_from_json():
    """Imports users from users.json into the SQLite database and adds default settings."""
    if not USERS_PATH.exists():
        print(f"[WARNING] {USERS_PATH} not found, skipping import.")
        return
    ...
    try:
        ...
        print(f"[OK] Users and settings imported to SQLite database.")
    except Exception as e:
        print(f"[ERROR] Error importing users: {e}")
    finally:
        conn.close()
def save_users_to_json():
    """Synchronizes users in SQLite to users.json file."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        ...
        print(f"[OK] {USERS_PATH} updated.")
    except Exception as e:
        print(f"[ERROR] Error saving users.json: {e}")
    finally:
        conn.close()
...
async def admin_required(current_user=Depends(get_current_user)):
    if current_user.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Unauthorized access (Admin privilege required)")
    return current_user
...
async def update_settings(req: SettingsUpdateRequest, current_user=Depends(get_current_user)):
    ...
    if not fields:
        raise HTTPException(status_code=400, detail="No data sent to update")
...
async def add_user(req: UserAddRequest, current_user=Depends(admin_required)):
    ...
    try:
        ...
        conn.commit()
        save_users_to_json()
        return {"message": "User added successfully", "username": req.username}
    finally:
        conn.close()
...
async def delete_user(username: str, current_user=Depends(admin_required)):
    ...
    try:
        cur = conn.cursor()
        # Check user existence first to avoid rollback of partial deletes
        cur.execute("SELECT username FROM users WHERE username = ?", (username,))
        if not cur.fetchone():
            raise HTTPException(status_code=404, detail="User not found")
            
        cur.execute("DELETE FROM user_settings WHERE username = ?", (username,))
        cur.execute("DELETE FROM access_requests WHERE username = ?", (username,))
...
async def emergency_request(req: EmergencyAuthRequest, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        return {"status": "success", "message": "Your emergency access request has been submitted. Pending admin approval."}
    finally:
        conn.close()
...
async def resolve_request(req: AccessRequestResolve, current_user=Depends(admin_required)):
    ...
    try:
        ...
        if req.status == 'approved':
            ...
            conn.commit()
            return {"message": f"Request approved. {hours} hours of access granted."}
        else:
            cur.execute("""
                UPDATE access_requests 
...
async def get_my_requests(current_user=Depends(get_current_user)):
    """Returns the user's own pending or active approved requests."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute("""
...
async def list_approved_requests(current_user=Depends(admin_required)):
    """Returns approved and still active (unexpired) access requests."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute("""
...
async def revoke_access(request_id: int, current_user=Depends(admin_required)):
    """Revokes an approved access (sets expires_at to past)."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute("""
...
async def start_presentation():
    ...
    return {"status": "success", "message": "10-minute presentation mode activated. IT and IoT anomalies are being simulated."}
...
async def stop_presentation():
    ...
    return {"status": "success", "message": "Presentation mode deactivated. System returned to real-time data."}
...
async def stream():
    ...
    return JSONResponse({
        ...
        "risk_scores": {
            ...
            "Finance": round(float(node_attn[2]) * 100, 1),
...
app.mount("/", StaticFiles(directory=str(BASE_DIR.parent / "frontend"), html=True), name="static")
